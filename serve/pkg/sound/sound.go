...
func CutSoundFile(name string, CheckPoint []float64, ID string) {
	//CheckPoint := [10]float64{0,1,2,3,4,5}
	wn := strings.Split(name, ".")
	name2 := "../../lib/wav/" + ID + "/" + wn[0] + ".wav"
	...
	for i := 0; i < len(CheckPoint)-1; i++ {
		...
		wav.WriteFile(b, "../../lib/wav/"+ID+"/"+wn[0]+"cut"+strconv.Itoa(i)+".wav")
		//uploadFile(os.Stdout, "cut"+strconv.Itoa(i)+".wav", "automediacheckpoint", "cut"+strconv.Itoa(i)+".wav")
		GCP.UploadFile(os.Stdout, "../../lib/wav/"+ID+"/"+wn[0]+"cut"+strconv.Itoa(i)+".wav", "automediacheckpoint", ID+wn[0]+"cut"+strconv.Itoa(i)+".wav")
	}
}
